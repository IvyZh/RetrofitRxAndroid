package cn.net.sinodata.ddj.ui;
 ...
 * 使用了我们自定义的FragmentTabHost，主要是因为官方提供的FragmentTabHost并没有进行优化，每次都会重新初始化一次Fragment。
 */
...
//					ft.detach(tab.fragment);
	...
	private FragmentTransaction doTabChanged(String tabId,
		...
		if (newTab == null) {
			throw new IllegalStateException("my No tab known for tag " + tabId);
		}
		if (mLastTab != newTab) {
			if (ft == null) {
				ft = mFragmentManager.beginTransaction();
